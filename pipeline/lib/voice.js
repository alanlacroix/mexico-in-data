...
- State the most likely outcome or direction without pretending certainty.
...
For the quarterly review, use two paragraphs. Paragraph one is view, evidence, and the better test. Paragraph two gives the expected direction, what would change the view, and the practical implication. First person is reserved for the quarterly review. Vary the wording naturally. "My concern is," "I expect," "I would revisit that view," and similar phrases are examples, not required openings. Never make every topic say "My base case" or close with the same sentence shape. Compact Briefly explained analysis preserves the same reasoning across Background, Our view, and What we're watching, but writes the likely outcome directly in ordinary language and then names the evidence that would prove it wrong. Start with the actual actor, event, or outcome. "The base case is" and "That view would change if" are permitted when they are genuinely the clearest wording, but they are never a required template and must not repeat across the Brief. Depth follows the question, not a fixed word target. A complicated story may need more room. If the inputs cannot support scale, a view, a mechanism, and a measurable change-of-mind condition, return no analysis. This is not permission to copy the same cadence or closing mechanically. Never fill the shape with generic prose.`;
